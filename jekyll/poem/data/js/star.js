...
function send_feedback_rhyme(){
    setTimeout(function(){
	send_feedback_core_rhyme();
    }, 100);
}

function send_feedback_core_rhyme(){
    $('#rate_message').html("");
    rhyme_id = $("#poem_id").html();
    if (poem_id == ""){
	console.log("no poem");
	return
    }
    ...
    $.ajax({
	url: "http://"+api_host+":"+port+"/api/feedback_rhyme",
	data: data,
	type:"GET",
	xhrFields: {
	    // The 'xhrFields' property sets additional fields on the XMLHttpRequest.
...
function init_star_rhyme(){
    $('#rate_star').css("display",'block');
    $('#rate_message').css("display",'block');
    $('#rate_star').raty({ starType: 'i', click: send_feedback_rhyme });
}
