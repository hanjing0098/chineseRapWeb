...
class Server:
    ...
    def __init__(self,port, model = 0, beam_size = 10, legacy_model = 1):
        self.host = '10.125.13.2' #socket.gethostname()
        print str(self.host)
        self.port = port
        self.model = model
        ...
        self.cmd = "{} --interactive 1 --adjacent-repeat-penalty -2.0 --repeat-penalty -3.0 -b {} -L 50 --decode-main-data-files {} -k 1 {} {} --fsa {} --print-beam {} --dec-ratio 0.0 10.0 --encourage-list dummy --encourage-weight 1.0".format(self.rnn, beam_size, self.source_path, self.model_path, self.kbest_path, self.fsa_path, self.print_beam)
    ...
    def start_server(self):
        ...
        backend = sp.Popen(cmd,bufsize=0,stdout=slave, stdin=sp.PIPE, cwd = self.run_dir)
...
p1 = Process(target=start_server_thread, args=(10011,0,50))
#p2 = Process(target=start_server_thread, args=(10011,0,200))
