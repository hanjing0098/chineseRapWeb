...
class Server:
    ...
    def __init__(self,port, model = 0, beam_size = 10, legacy_model = 1):
        ...
        self.cmd = "{} --interactive 1 --adjacent-repeat-penalty -2.0 --repeat-penalty -3.0 -b {} -L 160 --decode-main-data-files {} -k 1 {} {} --fsa {} --print-beam {} --dec-ratio 0.0 100.0 --encourage-list dummy --encourage-weight 1.0 --legacy-model {}".format(self.rnn, beam_size,  self.source_path, self.model_path, self.kbest_path, self.fsa_path, self.print_beam, legacy_model)
...
p1 = Process(target=start_server_thread, args=(10010,0,50))
