...
fsa_path_tp = os.path.join(root_dir, "gen_fsa/rap.fsa")
source_path_tp = os.path.join(root_dir, "gen_fsa/source.txt")
rhyme_path_tp = os.path.join(root_dir, "gen_fsa/rhyme.txt")
encourage_path_tp = os.path.join(root_dir, "gen_fsa/encourage.txt")
marjan_dir = os.path.join(root_dir, "gen_fsa/")
before_path_tp = os.path.join(root_dir, "gen_fsa/before.txt")
after_path_tp = os.path.join(root_dir, "gen_fsa/after.txt")
#ngram_path = os.path.join(root_dir,"models/5grams.txt") TODO
...
interactive_folder_tp = os.path.join(root_dir, 'gen_fsa_interactive/data')
...
def get_poem(k, model_type, topic, index=0, check=False, nline = None, no_fsa=False, style = None, withRhymeTable=True):
    ...
    #fsa_path = fsa_path_tp
    #source_path = source_path_tp
    #rhyme_path = rhyme_path_tp
    #encourage_path = encourage_path_tp
    ...
    if not no_fsa:
        ...
        print topic
        if topic is '':
          cmd = ["-t", u'战争', "-o", fsa_path, "-s", source_path]
          #cmd = ["bash", "run.sh", u'战争']
        else:
          cmd = ["-t", topic, "-o", fsa_path, "-s", source_path]
          #cmd = ["bash", "run.sh", topic]
        print cmd
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((host,fsa_port))
        message_old = s.recv(1024)
        assert(message_old == 'Accept')
        print host, fsa_port, message_old
        #sm.next_status(index)
        message = ' '.join(cmd)
...
class LogI(Resource): # log the interactive steps
    def get(self):
        ...
        #my_gcstore.log_poem_interactive(topic, nline, rhyme_id, poems, discourage_words, hme_flags,date, source = source) TODO
...
if __name__ == '__main__':
    ...
    #app.run(threaded=True, debug=True, host='hpc-login2.usc.edu', port=8080)
    app.run(threaded=True, debug=True, host='10.125.13.2', port=8080)
